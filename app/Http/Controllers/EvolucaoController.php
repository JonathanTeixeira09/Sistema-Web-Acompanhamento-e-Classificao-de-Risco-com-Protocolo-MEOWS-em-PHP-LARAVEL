...
//        dd($evolucao); // Debug: Verifique se a evolução está sendo carregada corretamente
...
//            ->whereBetween('created_at', [$filtroData['inicio'], $filtroData['fim']])
    ...
    public function salvarAvaliacao(Request $request, $evolucaoId)
    {
        $request->validate([
            ...
            'avaliacao.string' => 'A avaliação deve ser um texto.',
            'avaliacao.min' => 'A avaliação deve ter pelo menos 10 caracteres.',
            'conduta.required' => 'A conduta é obrigatória.',
            'conduta.string' => 'A conduta deve ser um texto.',
            'conduta.min' => 'A conduta deve ter pelo menos 10 caracteres.'
        ]);
